...
const categoryDropDown = document.getElementById("chooseACategory");

  const activityDropDown = document.getElementById("activities");

for (let i = 0; i < categories.length; i++) {
  const categoryOption = new Option(categories[i], categories[i]);
  categoryDropDown.appendChild(categoryOption);
}
categoryDropDown.onchange = generateActivitiesList;


  
function generateActivitiesList() {
  const userSelectedCategory = categoryDropDown.value;
  
 

  for (let i = 0; i < categories.length; i++) {
    if (activities[i].category == userSelectedCategory) {
      const activityInfo = new Option(activities[i].name, activities[i].id);
      activityDropDown.appendChild(activityInfo);
    }
  }
}
